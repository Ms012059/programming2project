...
void MusicPlayer::play() { // Method to play music
    try {
        if (currentsong >= 0 && currentsong < static_cast<int>(playlist.size())) {
            player->play();
...
void MusicPlayer::showPlaylist() {
    ...
    for (int i = 0; i < static_cast<int>(songname.size()); ++i) {
        list->addItem(songname[i]);
    }
    ...
    connect(playButton, &QPushButton::clicked, [&]() {
        ...
        if (row >= 0 && row < static_cast<int>(playlist.size())) {
            currentsong = row;
            player->setSource(playlist[row]);
            player->play();
            print("Playing: " + songname[row]);
            dialog.accept();
        }
    });
    ...
    connect(deleteButton, &QPushButton::clicked, [&]() {
        ...
        if (row >= 0 && row < static_cast<int>(playlist.size())) {
            ...
            songname.erase(songname.begin() + row);
...
void MusicPlayer::print(const QString &message) {
